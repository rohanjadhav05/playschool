...
export default function BranchInfoBlock({ branch, showMap = true }) {
  ...
  const showMapEmbed = showMap && !isPlaceholder(branch.maps?.embedUrl)
    ...
    <section className="py-12 md:py-16 bg-surface">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3">
            {branch.timing?.playschool && (
              <InfoRow
                ...
                label="Playschool timing"
                value={branch.timing.playschool}
              />
            )}
            {branch.timing?.tuition && (
              <InfoRow
                ...
                value={branch.timing.tuition}
              />
            )}
            {branch.batchSize?.playschool && (
              <InfoRow
                icon={Users}
                color="bg-primary-light text-textPrimary"
                label="Playschool batch"
                value={`Up to ${branch.batchSize.playschool} students`}
              />
            )}
            {branch.batchSize?.tuition && (
              <InfoRow
                ...
                label="Tuition batch"
                value={`Up to ${branch.batchSize.tuition} students`}
              />
            )}
            <InfoRow
              icon={Phone}
        ...
        {showMapEmbed ? (
          ...
          <div className="rounded-3xl shadow-card bg-gradient-to-br from-secondary-light to-bg min-h-[320px] flex flex-col items-center justify-center gap-4 p-8 text-center">
            <div className="w-16 h-16 bg-secondary rounded-2xl flex items-center justify-center">
              <MapPin size={28} className="text-white" />
            </div>
            <p className="font-display font-bold text-xl text-textPrimary">Find Us Here</p>
            <p className="font-body text-textSecondary text-sm">{addressLine}</p>
            {branch.maps?.shortUrl && (
              <a
                href={branch.maps.shortUrl}
                target="_blank"
